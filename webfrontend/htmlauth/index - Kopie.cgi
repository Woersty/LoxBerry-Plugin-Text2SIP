...
# T2S
our ($T2S_INSTALLED,$T2S_USE,$T2SPlugFolder,$T2SminVers);
...
# Read translations / phrases
  ...
  foreach our $template_string (@language_strings)
  {
	...
	# --- Temp-Logger nach Text2SIP.log ---
	my $DBG_LOG = 'REPLACELBHOMEDIR/log/plugins/text2sip/client_install.log';

	sub _dbg {
		my ($level, $msg) = @_;
		$level = defined $level ? uc($level) : 'INFO';
		$level = $level =~ /^(?:DEB|INFO|WARN|OK|ERR)$/ ? $level : 'INFO';
		$msg   = defined $msg ? "$msg" : '';

		my ($sec, $usec) = gettimeofday();
		my @lt = localtime($sec);
		my $ts = sprintf("%02d-%02d-%04d %02d:%02d:%02d.%03d",
			$lt[3], $lt[4]+1, $lt[5]+1900, $lt[2], $lt[1], $lt[0], int($usec/1000));

		my $dir = dirname($DBG_LOG);
		make_path($dir, { mode => 0755 }) unless -d $dir;

		# Optional: Sanitizer für Eingaben aus index.cgi/Wrapper
		$msg =~ s/^\[installer\]\s*//; # [installer] Präfix entfernen
		$msg =~ s/^\d{2}-\d{2}-\d{4}\s+\d{2}:\d{2}:\d{2}(?:\.\d{3})?\s+<[^>]+>\s+//; # vorhandenen Zeit/Level-Header entfernen

		# Mehrzeilig robust
		my @lines = split /\R/, $msg;

		if (open my $fh, '>>:encoding(UTF-8)', $DBG_LOG) {
			flock($fh, LOCK_EX);
			for my $line (@lines) {
				print $fh "$ts <$level> $line\n";
			}
			flock($fh, LOCK_UN);
			close $fh;
		}
	...
	# (0) Konfig-Prüfung: T2S_USE muss "on/true/1/yes" sein → sonst nichts tun
	my $cfg_file = ($lbpconfigdir && -d $lbpconfigdir)
		? "$lbpconfigdir/Text2SIP.cfg"
		: 'REPLACELBHOMEDIR/config/plugins/text2sip/Text2SIP.cfg';

	my $t2s_use_on = 0;
	eval {
		if (-r $cfg_file) {
			open my $CF, '<:encoding(UTF-8)', $cfg_file or die $!;
			my %kv;
			while (my $l = <$CF>) {
				next if $l =~ /^\s*(#|;)/ || $l =~ /^\s*$/;
				if ($l =~ /^\s*([A-Za-z0-9_]+)\s*=\s*(.+?)\s*$/) {
					my ($k,$v)=($1,$2); $v =~ s/^"|"$//g; $kv{$k}=$v;
				}
			}
			close $CF;
			my $val = $kv{T2S_USE} // $kv{t2s_use} // '';
			$t2s_use_on = ($val =~ /^(?:1|on|true|yes)$/i) ? 1 : 0;
		}
		1;
	} or do {
		_dbg('WARN', "Config read failed for $cfg_file: $@");
		$t2s_use_on = 0;
	};

	_dbg("T2S_USE (config)=" . ($t2s_use_on ? 'on' : 'off'));
	if (!$t2s_use_on) {
		_dbg("Skip install path: T2S_USE is OFF → no bundle/installer actions");
		# Früh raus aus diesem Block
		# (Rest der Seite läuft normal weiter)
	}
	else {
		
		# (1) T2S-Plugin vorhanden?
		$T2S_INSTALLED = "false" unless defined $T2S_INSTALLED;
		...
		my $t2s_is_installed = ($T2S_INSTALLED eq 'true') ? 1 : 0;
		_dbg("T2S_INSTALLED=" . ($t2s_is_installed ? 'true' : 'false') . " (min=$T2SminVers folder=$T2SPlugFolder)");

		# (2) Bundle-Check
		my $BUNDLE_DIR   = 'REPLACELBHOMEDIR/config/plugins/text2sip/bridge';
		my $BUNDLE_NAME  = 'sip_bundle.tar.gz';
		my $BUNDLE_PATH  = "$BUNDLE_DIR/$BUNDLE_NAME";

		my $SIP_INSTALLER = 'REPLACELBHOMEDIR/webfrontend/htmlauth/plugins/text2sip/bin/install_sip_client.pl';

		# ==== Main validator ====
		sub _bundle_seems_valid {
			my ($tar) = @_;

			return 0 unless defined $tar && -r $tar;

			my $list = `tar -tzf '$tar' 2>/dev/null`;
			my $rc   = $?;
			_dbg('DEB', "tar -tzf rc=$rc len=" . length($list));
			return 0 if $rc != 0 || !$list;

			my $has_master = ($list =~ m{(^|/)master\.info\b}m);
			my $has_ca     = ($list =~ m{(^|/)mosq-ca\.crt\b}m) || ($list =~ m{(^|/)ca\.crt\b}m);
			my $has_client =
				 ($list =~ m{(^|/)clients/[^/]+/client\.(?:crt|key)\b}m)
			  || ($list =~ m{(^|/)sip_bridge\.(?:crt|key)\b}m);

			_dbg('DEB', "bundle flags: master=" . ($has_master?1:0) . " ca=" . ($has_ca?1:0) . " client=" . ($has_client?1:0));

			my $ok = ($has_master && $has_ca && $has_client) ? 1 : 0;
			return 0 unless $ok;

			## Try to read MASTER_IP from bundle (overrides local IP)
			#if (my $ip = _read_master_ip_from_tar($tar)) {
			#	$MASTER_IP = $ip;
			#} else {
			#	_dbg('INFO', "Using local LoxBerry IP as MASTER_IP=$MASTER_IP");
			#}

			return $ok;
		}
		_dbg("Bundle path: $BUNDLE_PATH exists=" . (-e $BUNDLE_PATH ? 1:0) . " readable=" . (-r $BUNDLE_PATH ? 1:0));
		_dbg("Installer path: $SIP_INSTALLER exists=" . (-e $SIP_INSTALLER ? 1:0) . " readable=" . (-r $SIP_INSTALLER ? 1:0));

	my $did_install = 0;
	my $install_exit;

	if (!$t2s_is_installed && _bundle_seems_valid($BUNDLE_PATH)) {
		_dbg("Conditions met → running install_sip_client.pl --debug at $SIP_INSTALLER");

		my $cmd = "perl '$SIP_INSTALLER' --debug 2>&1";
		my $out = `$cmd`;
		my $rc  = $?;                 # raw rc
		$install_exit = $rc >> 8;     # exit code
		$did_install  = 1;

		for my $line (split /\r?\n/, $out // '') {
			_dbg("[installer] $line");
		}
		_dbg("install_sip_client.pl rc_raw=$rc exit=$install_exit");

		# Nur wenn wirklich Installation lief, baue das Popup
		if ($install_exit == 0) {
			$POPUP = flash_popup(
			  message      => "TLS package for MQTT has been installed successfully.\n\nClient certificates are ready.",
			  title        => "Text2SIP - TLS",
			  icon         => '/plugins/text2sip/Text2SIP_ok.png',
			  psubfolder   => '', 
			  autoclose_ms => 8000,
			  reload       => 1
			);
			...
			$POPUP = flash_popup(
			  message      => "TLS package for MQTT installation failed (exit $install_exit).\n\nSee $DBG_LOG for details.",
			  title        => "Text2SIP - TLS",
			  icon         => '/plugins/text2sip/Text2SIP_fail.png',
			  psubfolder   => '',
			  autoclose_ms => 10000,
			  reload       => 0
			);
		...
		_dbg("Skip install: T2S_INSTALLED=".($t2s_is_installed?'true':'false')
			." bundle_valid=".(_bundle_seems_valid($BUNDLE_PATH)?1:0));
	}
...
##########################################################################
  ...
  if ($do eq "makecall")
  {
	...
	if ($cfg_flag eq 'on') {
		...
		# Wenn t2svoice keine WAV erzeugt hat, Fallback (explizit loggen)
		if (!-e $pluginwavfile || -s $pluginwavfile <= 0) {
...
# ========================================
# t2svoice - Unified TTS function for SIP
# Final version: MQTT local publish only + fallback
# ========================================

sub t2svoice {
 
    # ============================================================
    # Local safety init (never depend on outer scope)
    # ============================================================
    ...
    if (open my $fh, '>>', $log_path) {
        print $fh _ts() . " ## DEBUG: t2svoice START (log=$log_path client=$psubfolder)\n";
        close $fh;
    } else {
        open my $fh, '>>', '/tmp/t2svoice_fallback.log';
        print $fh _ts() . " ## FATAL: cannot open $log_path for writing\n";
        close $fh;
    }
    ...
    # ---------- Fallback ----------
    $log->("## Fallback to pico2wave (all MQTT attempts failed)");
    return usepico();
}
